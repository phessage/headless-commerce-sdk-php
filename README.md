...
Password recovery uses `requestCustomerPasswordRecovery()` and `resetCustomerPassword()`. Requests are deliberately non-enumerating; a reset capability expires after one hour, works once and must never be logged.
