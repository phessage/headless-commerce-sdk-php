...
if($customerEmail!==''&&$customerPassword!==''){
    ...
    $returnIntent='customer-return:'.(getenv('GITHUB_RUN_ID')?:bin2hex(random_bytes(12)));$returnInput=['reason'=>'not_as_expected','items'=>[['orderItemId'=>$returnOrderItemId,'quantity'=>1,'resolution'=>'refund']]];$return=$client->createCustomerReturn($auth['token'],$returnOrderId,$returnInput,$returnIntent)['data']['return'];$assert(($return['orderId']??'')===$returnOrderId&&($return['status']??'')==='requested','return creation projection drifted');$replayed=$client->createCustomerReturn($auth['token'],$returnOrderId,$returnInput,$returnIntent)['data']['return'];$assert(($replayed['id']??null)===($return['id']??null),'return creation did not replay the original request');try{$client->createCustomerReturn($auth['token'],$returnOrderId,[...$returnInput,'note'=>'different intent'],$returnIntent);throw new RuntimeException('return key reuse with a different payload did not conflict');}catch(ProblemException $error){$assert($error->status===409,'different return intent did not return 409');}
