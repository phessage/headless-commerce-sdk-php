...
final class Client {
    ...
    public function requestCustomerPasswordRecovery(string $email): array { return $this->customerRequest('POST','/auth/password/recovery',null,['email'=>$email]); }
    public function resetCustomerPassword(string $email,string $token,string $newPassword): array { return $this->customerRequest('POST','/auth/password/reset',null,['email'=>$email,'token'=>$token,'newPassword'=>$newPassword]); }
    ...
    public function authorizeCustomerOAuth(string $provider,string $redirectUri,string $codeChallenge,string $state): array { if(!in_array($provider,['google','apple'],true))throw new \InvalidArgumentException('OAuth provider must be google or apple');if(strlen($state)<16||strlen($state)>1024)throw new \InvalidArgumentException('OAuth state must contain 16-1024 characters');return $this->customerRequest('POST','/auth/oauth/'.rawurlencode($provider).'/authorize',null,['redirectUri'=>$redirectUri,'codeChallenge'=>$codeChallenge,'state'=>$state]); }
    public function exchangeCustomerOAuth(string $code,string $codeVerifier,string $redirectUri,?string $cartToken=null): array { return $this->customerRequest('POST','/auth/oauth/token',null,['code'=>$code,'codeVerifier'=>$codeVerifier,'redirectUri'=>$redirectUri],$cartToken); }
